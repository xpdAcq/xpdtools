...
li1 = LiveImage('image', cmap='viridis',
                limit_func=lambda x: (np.nanpercentile(x, .1),
                                      np.nanpercentile(x, 99.9)),
                # norm=SymLogNorm(.1),
...
print(geo.poni1, geo.poni2)
raw_foreground_dark.emit(0.0)
raw_background_dark.emit(0.0)
raw_background.emit(0.0)

ave_ff.map(lambda x: ((ff / x) - 1) * 100).map(np.nan_to_num).sink(li1.update)
mean_array.map(np.nan_to_num).sink(li2.update)
raw_foreground.map(np.nan_to_num).sink(li3.update)
...
for i, p in enumerate(np.linspace(.2, -.2, k)):
    for j, pp in enumerate(np.linspace(.2, -.2, k)):
        ...
        print(p, pp, new_geo.poni1, new_geo.poni2)
