...
def findrings(image):
	...
	def findringcenter(image,thres=0.2,d=20):
		...
		Parameters:
			image: numpy array of tiff image
			thres: float representing percentage of image maximum that could be considered a value of data on inner ring
		...
		def findthreshold(image,thres):
			"""
			Takes in a numpy array corresponding to tiff image and a percentage of the maximum pixel value and returns the maximum pixel value.
			...
			Returns:
				float corresponding to a percentage of the maximum value in image numpy array
